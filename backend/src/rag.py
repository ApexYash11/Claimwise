...
def chunk_texts(
    texts: List[str], chunk_size: int = 500, overlap: int = 50
) -> List[str]:
    """
    ...
    while i < len(words):
        ...
        if len(chunks) > 10000:  # Reasonable upper limit
            logger.warning(
                f"Stopping chunking at {len(chunks)} chunks to prevent memory issues"
            )
            break
...
async def index_documents(
    text: str, document_id: str, chunk_size: int = 500, overlap: int = 50
    ...
    logger.info(
        f"Filtered text length: {len(filtered_text)} characters (saved {len(text) - len(filtered_text)} chars)"
    )
    ...
    logger.info(
        f"After deduplication: {len(unique_chunks)} unique chunks (saved {len(chunks) - len(unique_chunks)} duplicates)"
    )

    # Step 4: Filter chunks worth embedding
    quality_chunks = [chunk for chunk in unique_chunks if should_embed_chunk(chunk)]
    logger.info(
        f"After quality filtering: {len(quality_chunks)} chunks worth embedding (filtered out {len(unique_chunks) - len(quality_chunks)} low-quality chunks)"
    )
    ...
    try:
        embs = await embed_texts(quality_chunks)
        if embs is None:
            raise RuntimeError(
                "Embedding API returned None - check GEMINI_API_KEY and API status"
            )
    ...
    if len(embs) < len(quality_chunks):
...
async def retrieve_top_k(
    query: str, k: int = 5, policy_id: Optional[str] = None
