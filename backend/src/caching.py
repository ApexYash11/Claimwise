...
CACHE_PERSIST_DIR = os.getenv("CACHE_PERSIST_DIR", "")
...
class AdvancedCache:
    """Advanced in-memory cache with multiple eviction strategies and optional file persistence"""
    ...
    def _choose_eviction_key(self) -> Optional[str]:
        ...
        if self.strategy == CacheStrategy.LRU:
            ...
            expired_keys = [
                k for k, v in self._cache.items() if v.is_expired(current_time)
            ]
            if expired_keys:
                return min(expired_keys, key=lambda k: self._cache[k].created_at)
            else:
    ...
    @property
    def _persist_path(self) -> Optional[str]:
        if not CACHE_PERSIST_DIR or not self._name:
            return None
        return os.path.join(CACHE_PERSIST_DIR, f"cache_{self._name}.pkl")
    ...
    def save_to_disk(self):
        ...
        try:
            ...
            logger.debug("Failed to persist cache %s: %s", self._name, e)
    ...
    def load_from_disk(self):
        ...
        try:
            ...
            with self._lock:
                self._cache.clear()
                self._access_order.clear()
                for k, item_data in snapshot.get("cache", {}).items():
                    item = CacheItem(
                        key=item_data["key"],
                    ...
                    if not item.is_expired():
                        self._cache[k] = item
                        self._access_order[k] = True
                        self._current_memory_usage += item.size_bytes
                self._hits = snapshot.get("hits", 0)
                self._misses = snapshot.get("misses", 0)
                self._evictions = snapshot.get("evictions", 0)
            logger.info(
                "Loaded %d items from disk cache %s", len(self._cache), self._name
            )
        except (OSError, pickle.PickleError, KeyError, ValueError) as e:
            logger.debug("Failed to load cache %s from disk: %s", self._name, e)
