"""
Performance monitoring with request tracking, system metrics collection, and analytics.
"""
...
class PerformanceMonitor:
    ...
    def record_system_metric(self):
        ...
        try:
            ...
            with self.lock:
                ...
                error_count = sum(
                    1 for m in recent_metrics if m.status_code >= 400 or m.error
                )
    ...
    def get_performance_summary(self) -> Dict[str, Any]:
        ...
        with self.lock:
            ...
            recent_requests = [
                m for m in self.request_metrics if m.timestamp >= hour_ago
            ]
            ...
            error_count = sum(
                1 for m in recent_requests if m.status_code >= 400 or m.error
            )
            ...
            return {
                ...
                "slow_requests": len(
                    [
                        m
                        for m in recent_requests
                        if m.response_time > self.slow_request_threshold
                    ]
                ),
            }
