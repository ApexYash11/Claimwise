"""
Token bucket and sliding window rate limiter with per-user and per-IP throttling.
"""
    ...
    def is_allowed(self, key: str, config: RateLimitConfig) -> RateLimitResult:
        ...
        with self._lock:
            ...
            if len(request_times) >= config.max_requests:
                ...
                retry_after = int(
                    oldest_request + config.window_seconds - current_time + 1
                )
                reset_time = datetime.fromtimestamp(
            ...
            if request_times:
                reset_time = datetime.fromtimestamp(
                    request_times[0] + config.window_seconds
                )
            else:
                reset_time = datetime.fromtimestamp(
                    current_time + config.window_seconds
                )
    ...
    def is_allowed(self, key: str, config: RateLimitConfig) -> RateLimitResult:
        ...
        with self._lock:
            ...
            if bucket["tokens"] >= 1.0:
                ...
                time_to_full = (
                    config.max_requests + config.burst_allowance - bucket["tokens"]
...
class RateLimitManager:
    ...
    def __init__(self):
        ...
        self._stats: Dict[str, Dict[str, Any]] = defaultdict(
            lambda: {"requests": 0, "blocked": 0, "last_reset": datetime.now()}
        )
        self._lock = threading.Lock()
    ...
    def check_rate_limit(
        self, limit_name: str, identifier: str, endpoint: Optional[str] = None
    ) -> RateLimitResult:
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            for name, stats in self._stats.items():
                total_requests = (
                    int(stats["requests"]) if isinstance(stats["requests"], int) else 0
                )
                blocked_requests = (
                    int(stats["blocked"]) if isinstance(stats["blocked"], int) else 0
                )
                block_rate = (
