    ...
    async def embed_texts(self, texts: List[str]) -> List[List[float]]:
        ...
        try:
            ...
            raise ExternalAPIError(
                message="OpenAI embedding timed out after 15s",
    ...
    async def embed_texts(self, texts: List[str]) -> List[List[float]]:
        ...
        try:
            result = await asyncio.to_thread(
                lambda: self._genai.embed_content(  # type: ignore
...
class SentenceTransformerProvider(EmbeddingProvider):
    ...
    async def embed_texts(self, texts: List[str]) -> List[List[float]]:
        """Generate embeddings using sentence transformers (non-blocking via to_thread)"""
        import asyncio
        ...
        try:
            embeddings = await asyncio.wait_for(
                asyncio.to_thread(
                    lambda: self._model.encode(texts, convert_to_tensor=False)
                ),
                timeout=10.0,
            )
            ...
            raise ProcessingError(
                message="Sentence transformer embedding timed out after 10s",
                operation="generate_embeddings",
            )
        except Exception as e:
            raise ProcessingError(
...
class HuggingFaceEmbeddingProvider(EmbeddingProvider):
    ...
    async def embed_texts(self, texts: List[str]) -> List[List[float]]:
        """Generate embeddings using Hugging Face models (non-blocking via to_thread)"""
        import asyncio
        ...
        if self._model is None:
            raise ProcessingError(
                message="Hugging Face model failed to load",
                operation="generate_embeddings",
            )
        ...
        try:
            ...
            embeddings = await asyncio.to_thread(lambda: self._encode_hf(encoded_input))
    ...
    def _encode_hf(self, encoded_input) -> List[List[float]]:
        import torch
...
class EmbeddingManager:
    ...
    async def embed_texts(
        self, texts: List[str], provider: Optional[str] = None, use_cache: bool = True
    ) -> List[List[float]]:
        """Generate embeddings for a list of texts"""
        ...
        try:
            ...
            logger.debug(
                f"Generated {len(embeddings)} embeddings using {provider_name}"
            )
            return embeddings
    ...
    async def embed_single_text(
        self, text: str, provider: Optional[str] = None, use_cache: bool = True
    ) -> List[float]:
...
async def embed_texts(
    texts: List[str], provider: Optional[str] = None, use_cache: bool = True
) -> List[List[float]]:
    """Generate embeddings for a list of texts"""
...
async def embed_text(
    text: str, provider: Optional[str] = None, use_cache: bool = True
) -> List[float]:
    """Generate embedding for a single text"""
